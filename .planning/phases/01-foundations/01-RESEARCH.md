...
| A2 | **RESOLVED — deferred-to-Phase-2 (option c) — daemon captures all 4 Anthropic Usage fields verbatim; Phase 2 cost worker calibrates against real billed-usage before locking the formula; verified 2026-05-31** against [Anthropic prompt-caching docs](https://platform.claude.com/docs/en/build-with-claude/prompt-caching) — original claim was "Anthropic Usage object semantics: `cache_creation_input_tokens` and `cache_read_input_tokens` are SEPARATE from `input_tokens` (formula: total_input = read + creation + input)" | Canonical schema, Pitfall 6 | **Risk neutralised for Phase 1.** The Phase 1 daemon writes all 4 fields VERBATIM (`input_tokens`, `output_tokens`, `cache_creation_input_tokens`, `cache_read_input_tokens`) into `payload.usage` with no aggregation. Phase 2's cost worker will (a) sample a few real Claude Code sessions with cache hits, (b) compare summed-vs-input-only interpretations against Anthropic's billed-usage dashboard for the same period, (c) lock the empirically-correct formula. Until that calibration ships, no Phase 1 component computes totals, so no live system can miscount. Downstream Phase 1 tests (Plan 01-07 hook adapter, Plan 01-10 smoke) assert only that the 4 fields are present + non-negative ints — they do NOT assert any math invariant. |
