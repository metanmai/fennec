...
**CLAIM A4.2 [HIGH — verified live POC, refined per EVIDENCE C11] — event_type derivation:** `.git/logs/HEAD` reflog lines carry a trailing message that discriminates the event:
- `commit: <subject>` → `event_type = "commit"`.
- A commit whose `<subject>` matches `^Revert "` → `event_type = "revert"` (the PRIMARY, version-robust D2-06 v1 rule — works regardless of the reflog prefix). ALSO accept BOTH reflog prefixes as a secondary signal: `revert:` (the form **modern git** emits, observed live: `HEAD@{1}: revert: Revert "second change"`) AND the older/alternate `commit (revert):`.
...
*Falsify:* `git -c core.editor=true revert --no-edit HEAD` then `git reflog -5` shows `HEAD@{1}: revert: Revert "<subject>"` (the `revert:` prefix on modern git); the `Revert "` commit-subject rule catches it across git versions.
...
**CLAIM B5.2 [HIGH — verified live POC] — chatSessions JSON shape:** Each `chatSessions/*.json` (observed `version:3`) has top-level `{version, requesterUsername, responderUsername, requests[], sessionId, creationDate, lastMessageDate}`. Each `requests[i]` has `{requestId, message:{text, parts}, response[], responseId, result:{errorDetails, timings, metadata:{modelMessageId, responseId, sessionId, agentId}}, timestamp, modelId, agent, contentReferences}`. The prompt is `requests[i].message.text`; the model is `requests[i].modelId`; the response is `requests[i].response`.
*Falsify:* `jq '{version, req0keys: (.requests[0]|keys), text: .requests[0].message.text, model: .requests[0].modelId}' <chatSessions json>`.

> **`modelId` normaliser caveat [HIGH — verified live POC, refines B5.2 / EVIDENCE C42]:** `modelId` is NOT always present and its prefix is NOT uniform. Across 9 sessions / 99 requests, 98/99 carried `modelId` (one was `null` — likely a canceled/in-flight request) with prefixes `copilot/` AND `github.copilot-chat/` (observed `copilot/gpt-4.1`, `github.copilot-chat/gpt-4.1`, `copilot/claude-sonnet-4`). The normaliser MUST (a) tolerate a MISSING `modelId` (skip the row or tag it `unknown`, never throw) and (b) strip EITHER prefix (`copilot/` or `github.copilot-chat/`) to get the bare model name for the model→tier lookup. *Falsify:* `jq '[.requests[].modelId]' <chatSessions json>` shows mixed prefixes and at least one null across a multi-session sample.
