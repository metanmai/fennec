...
2. **The backend analysis layer**: wire ingest to enqueue onto a Cloudflare Queue (un-commenting the binding stub in `backend/wrangler.jsonc`), then ONE queue-consumer Worker that runs **correlation** and **rule-based model-fit** as two idempotent functions over each batch (Cloudflare allows only one push consumer per queue — D2-14), plus a separate **daily aggregator cron** — and the migrations for `prompt_outcomes`, `model_fit_scores`, `daily_rollups_by_user`, `daily_rollups_by_project`, and `model_pricing`. Plus the `fennec inspect` transparency CLI.
...
- **D2-06: Git-watcher emits `git_events`, not `ai_events`,** with `event_type ∈ {commit, revert, file_edit, branch_switch}` (the locked CHECK constraint in `20260531000003_git_events_partitioned.sql`) and carries `repo_remote`/`repo_branch`/`occurred_at`. Revert detection v1 = a commit whose subject matches Git's default `Revert "<subject>"` form (the primary, version-robust rule) OR a `git revert` reflog entry, accepting BOTH reflog prefixes — `revert:` (the form modern git emits) and the older `commit (revert):` (see RESEARCH A4.2 / EVIDENCE C11); richer semantic revert detection is deferred. The `EventKindSchema` in `@fennec/shared` is for `ai_events` kinds and does NOT need git kinds added (git kinds live in the table CHECK) — so no `schema_version` bump is required for the git-watcher.
...
- **D2-14: ONE events queue + ONE consumer Worker that runs correlation and model-fit as two idempotent functions within the same batch handler.** Cloudflare Queues allow only ONE push consumer per queue (RESEARCH C7.4 / EVIDENCE C36 / OPEN-QUESTIONS Q14), so correlation and model-fit canNOT be two independent push consumers of one queue. Instead, the single consumer Worker invokes both functions over each batch — each does idempotent upserts (D2-15) so a redelivery re-derives the same `prompt_outcomes` / `model_fit_scores` rows. The aggregator is NOT a queue consumer — it stays a scheduled cron that reads the already-written rows. The self-host equivalent (pg-backed queue / graphile-worker, ING-07/Phase 6) runs the same two functions in one worker. Forward-reference: if Cloudflare adds multi-consumer support, these two functions could split into separate consumers (OPEN-QUESTIONS Q14).
...
- **D2-27: Subscription products are a separate cost line, never summed into per-token `cost_estimated` (ANL-09).** Represent fixed-period subscriptions (GitHub Copilot Pro ~$10/mo; ChatGPT **Plus** $20/mo) as their own SEED rows in the `model_pricing` effective-date table — either a `pricing_kind` discriminator in `model_pricing` (`per_token` vs `subscription`) or a sibling `subscription_pricing` table. The rollup surfaces a distinct `cost_subscription` field. A test confirms subscription cost is NOT added into `cost_estimated`. **STRONG** (SPEC). Planner's call on same-table-discriminator vs sibling-table; same-table-discriminator is the leaner default. *Pricing note:* Copilot Pro is **$10/mo** (NOT the $19 originally drafted) and moved to usage-based/credit billing on 2026-06-01, so a flat-subscription line is a deliberate simplification; ChatGPT **Pro** is the $200/mo tier (the $20 figure is ChatGPT **Plus**). These live prices are volatile — re-verify subscription prices at build time (see OPEN-QUESTIONS Q12/Q15). This is a seed-value + wording correction only; the schema/architecture is unchanged.
